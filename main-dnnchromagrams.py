...
for epoch in range(num_epochs):
    ...
    for arch_ in range(arch_length):
        ...
        sys.stdout.write('\r Epoch: ' + str(epoch + 1) + ' de ' + str(num_epochs) + '. Procesando archivo ' + str(arch + 1) + ' de ' + str(arch_length) + \
                         '. alpha = ' + str(alpha_lnqt))
        # cont += 1
        ...
        '''
        #if arch_ in [0, 25, 50, 75, 100]:
        sys.stdout.write('\rEpoch: ' + str(epoch + 1) + ' de ' + str(num_epochs) + '. Procesando archivo ' + str(arch_ + 1) + ' de ' + str(arch_length) + '. alpha = ' + str(alpha_lnqt))
        filename = archivo[arch_, 0]
        ...
        toc_dnn = time.time()
        # print " tiempo dnn: ", str(toc_dnn - tic_dnn)
        # tiempo_ind = time.time() - tec
        # tiempo_ejec += tiempo_ind
        # tiempo_ejecucion = tiempo_ejec / (arch_ + 1)
        # sys.stdout.write('\r tiempo ejecucion: ' + str(tiempo_ejecucion))
        # print " tiempo ejecución individual : ", tiempo_ind, ",  promedio: ", tiempo_ejecucion
    ############################################################
    #                  DNN - Cross-validation                  #
    ############################################################
    # toc_epoch = time.time() - tic_epoch
    # print " tiempo epoch: ", str(toc_epoch)
    '''
    os.chdir("..")
    if training_dataset == 'Original':
        ...
        os.chdir('Database')
    if training_dataset == 'Anecoica':
        validation_files = np.genfromtxt('Anecoica_regrabacion_d32cm/cross-val-set.txt', delimiter='; ', dtype='str',
                                         skip_header=1)
    ...
    for val in range(val_length):
        ...
        error_net = tf.divide(-tf.reduce_sum(tf.add(tf.multiply(out_gt, tf.log(out_layer+eps)),
                                                    tf.multiply(1-out_gt+eps, tf.log(1-out_layer+eps))), 0),
                              tf.constant(value=12.0))
        error_val += np.sum(error_net.eval(feed_dict={in_layer: supervector_matrix, out_gt: ground_truth})) / n_frames
        error_val_cross += np.sum(cross_entropy.eval(feed_dict={in_layer: supervector_matrix, out_gt: ground_truth})) /\
                            n_frames
    error_val = error_val / val_length
    error_val_cross = error_val_cross / val_length
    learning[epoch] = error_val
    print ""
    print "Error epoch "+str(epoch + 1)+': Error_net: '+str(error_val) + '. error_val: '+str(error_val_cross)
    '''
